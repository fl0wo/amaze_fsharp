...
let (|C|_|) (s: string, i) =
    if i < s.Length then Some(s.[i], (s, i + 1))
    else None

let numeric = set [ '0' .. '9' ] + set [ '+'; '-'; '.'; 'e'; 'E' ]
let whitespace = set [ ','; ':'; '\t'; '\n'; '\r'; ' ' ]

let (|Contains|_|) alphabet =
    function
...
let rec (|Lex|_|) =
    ...
    | LexNumber((s, last) as it) & (_, first) -> Some(LITERAL(Number(float s.[first..last - 1])), it)
...
let rec (|ParseJSON|_|) =
    ...
    | Lex(ARRAY Open, Star (|ParseJSON|_|) (jsons, Lex(ARRAY Close, it))) -> Some(Array jsons, it)
    | Lex(OBJECT Open, Star (|ParseMember|_|) (members, Lex(OBJECT Close, it))) -> Some(Object members, it)
    | _ -> None
