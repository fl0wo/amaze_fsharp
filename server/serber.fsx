...
let writeToClient cl msg =
    ...
    async {
        do! streamWriter.WriteLineAsync(msg.ToString())
...
let forwadMessageToConnectedClients msg = clientTaskList |> List.map (fun (_, _, _, stream) -> writeToClient stream msg)
            ...
            try
                while true do
                    ...
                    do! forwadMessageToConnectedClients (endpoint + ": " + line)
                        ...
                        |> Async.Ignore
            with _ ->
                let client = clientTaskList |> List.find (fun (id, _, _, _) -> id = clientId)
        ...
        async {
            ...
            while true do
                ...
                let clientListenTask =
                    Async.StartAsTask
                        (listenForMessages cmd_parser id endpoint (client.GetStream()), cancellationToken = cts.Token)
